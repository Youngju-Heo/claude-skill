...
구성은 4가지입니다: `CLAUDE.md`(판단 원칙), `dev-workflow`(개발 절차),
`devops-workflow`(Azure DevOps 일감 연동, 선택), `.claude/rules/`(언어별
코드 규칙).
...
- **스킬 내 추가 사항**: "최종리뷰 1회" 유지(과잉 검증 방지), "심각도 무관
  ...
  추가(길어진 출력 성향 대응).
- **경량 경로 추가**: 설계 결정이 없고 변경이 한 관심사에 국한되면 4단계
  대신 3스텝(보고 → 구현 → 요약)으로 끝냅니다. 오타 수정이나 상수 변경에도
  설계·계획 문서가 생기는 것을 막습니다. TDD와 커밋 규칙은 그대로 유지되고,
  판단이 애매하면 전체 경로가 기본값입니다.
...
로드되어, 워크플로를 타지 않는 세션에서는 코드 규칙이 빠지게 됩니다.)
...
Work Item 조회·요구사항 요약 후 상태 전환 여부를 확인하고, 완료 상태
전환도 반드시 사용자 확인을 받은 뒤 수행합니다. 요구사항 확정 컨펌은
dev-workflow의 기획/설계 단계에서 받으므로 중복되지 않습니다.
...
선택 방법: CLAUDE.md의 "7. 기능 개발 워크플로" 섹션에 지시문 두 종류가
있습니다(일반 프로젝트용 / Azure DevOps 프로젝트용). 프로젝트에 맞는
쪽만 남기고 다른 쪽은 HTML 주석(`<!-- -->`)으로 감싸 두세요.
기본 상태에서는 일반 지시문이 활성 줄과 주석 사본 두 벌로 들어 있습니다 —
전환할 때 주석 위치만 옮기면 되도록 남겨둔 것입니다.
...
1. Work Item(+ 부모 항목·댓글) 조회 → 요구사항 요약
2. 사용자 확인 후 상태를 진행 중으로 전환, `{id}-{feature-name}` 브랜치 생성
3. dev-workflow 절차 수행 (인수 조건 → 설계 문서의 성공 기준으로 반영,
   설계·계획 문서명에 `{id}` 삽입, 커밋 메시지에 `#{id}` 포함해
   Work Item 연결)
...
- 간단한 변경(오타·상수 수정)에 4단계가 통째로 돌지 않는지 — 돌면 경량 경로
  조건이 너무 좁은 것이고, 반대로 설계가 필요한 작업이 경량 경로로 새면
  조건이 너무 넓은 것입니다
